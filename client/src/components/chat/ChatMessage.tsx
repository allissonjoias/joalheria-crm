...
export function ChatMessage({ papel, conteudo, criado_em }: ChatMessageProps) {
  ...
  return (
    <div className={`flex gap-3 ${isAssistant ? '' : 'flex-row-reverse'}`}>
      <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${isAssistant ? 'bg-alisson-600' : 'bg-gray-600'}`}>
      ...
      <div className={`max-w-[70%] ${isAssistant ? '' : 'text-right'}`}>
        <div className={`inline-block px-4 py-3 rounded-2xl text-sm leading-relaxed ${isAssistant ? 'bg-white border border-gray-100 text-gray-800 rounded-tl-none' : 'bg-alisson-600 text-white rounded-tr-none'}`}>
          {conteudo.split('\n').map((line, i) => (
